...
def validate_model(test_loader, ann, print_freq=100):
    ...
    print(
        f" * Prec@1 {final_top1:.3f}\tPrec@5 {final_top5:.3f}\t\Loss {final_loss:.3f}\ttotal_num={total_num.item()}\t"
    )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader, test_loader = build_imagenet_data(
        ...
        dist_sample=(ngpus_per_node > 1),
    )

    cali_data = get_train_samples(
        train_loader, num_samples=int(args.num_samples / ngpus_per_node)
    )
    torch.backends.cudnn.benchmark = False
