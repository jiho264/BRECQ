...
def block_reconstruction(
    ...
    if not act_quant:
        ...
        for name, module in block.named_modules():
            if isinstance(module, QuantModule):
                ...
                """[My comment]
                [main - blockrRec - (1) init AdaRoundQuantizer]
    ...
    for i in range(iters):
        ...
        optimizer.zero_grad()
        """[My comment] [main - blockRec - (2) forward]
        >>> out_quant == x_float_q
        """
        out_quant = block(cur_inp)

        """[My comment] [main - blockRec - (3) get loss]
    ...
    """[My comment] [main - blockRec - (5) END]
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.rec_loss == "mse":
            """[My comment] [main - blockRec - (3) get loss - (1) get L2 norm loss]
                L2norm(f(Wx) - f(W_qx))    ...(the part of eq. 25)
            >>> rec_loss (the SUM about layers in block)
            """
        ...
        if self.count < self.loss_start or self.round_loss == "none":
            ...
            round_loss = 0
            """[My comment] [main - blockRec - (3) get loss - (2) get h(V_(i,j)]
            >>> return h(V_(i,j))
            """
            for name, module in self.block.named_modules():
                if isinstance(module, QuantModule):
                    ...
                    """[My comment] [main - blockRec - (3) get loss - (3) get lambda * f_reg(V]
                    ...
                    >>> return sum(lambda * f_reg(V_(i,j)))
                    """
                    round_loss += (
        ...
        """[My comment]
        [main - blockRec - (3) get loss - (4) get total loss]
            argmin_(V) [L2norm(f(Wx) - f(W_qx)) + lambda * f_reg(V_(i,j))]    ...(eq. 25)
        >>> return loss
        if act_quant,
        >>> return L2norm(f(Wx) - f(W_qx))
        (just apply LSQ with Adam optim. no need to optimize V. the loss from V is always 0.)
        """
